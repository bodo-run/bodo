...
fn test_execution_plugin_command_failure() -> Result<()> {
    ...
    let node_id = graph.add_node(NodeKind::Task(TaskData {
        name: "test_task".to_string(),
        description: None,
        command: Some("nonexistent_command_that_should_fail".to_string()), // Command that fails
        working_dir: None,
        env: HashMap::new(),
        exec_paths: vec![],
        arguments: vec![],
...
fn test_run_concurrent_group() -> Result<()> {
    ...
    let child_task1_id = graph.add_node(NodeKind::Task(TaskData {
    ...
    let fail_command = "nonexistent_command_that_should_fail"; // Command that fails
