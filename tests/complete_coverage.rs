...
impl bodo::Plugin for Dummy {
    ...
    fn on_init(&mut self, _config: &bodo::plugin::PluginConfig) -> bodo::Result<()> {
        Ok(())
    }
    fn on_graph_build(&mut self, _graph: &mut Graph) -> bodo::Result<()> {
        Ok(())
    }
    fn on_after_run(&mut self, _graph: &mut Graph) -> bodo::Result<()> {
        Ok(())
    }
    fn on_run(&mut self, _node_id: usize, _graph: &mut Graph) -> bodo::Result<()> {
        Ok(())
    }
}
