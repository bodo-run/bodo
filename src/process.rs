...
impl ProcessManager {
    ...
    pub fn run_concurrently(&mut self) -> Result<(), Box<dyn Error>> {
    ...
    pub fn kill_all(&self) -> Result<(), Box<dyn Error>> {
...
fn dummy_child() -> Result<Child, Box<dyn Error>> {
...
mod tests {
    ...
    fn test_concurrent_run() {
        let mut pm = ProcessManager::new(true); // fail_fast = true
        pm.spawn_command(
            "task1",
            "echo 'Task1 start' && sleep 0.2 && echo 'Task1 done'",
            None,
        )
        .unwrap();
        pm.spawn_command("task2", "echo 'Task2 start' && sleep 0.1 && false", None)
            .unwrap(); // fails
        pm.spawn_command(
            "task3",
            "echo 'Task3 start' && sleep 0.3 && echo 'Task3 done'",
            None,
        )
        .unwrap();
        let result = pm.run_concurrently();
        assert!(result.is_err(), "Should fail because task2 fails quickly");
    }
}
